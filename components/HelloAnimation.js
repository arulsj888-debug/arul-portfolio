...
import styles from '../styles/HelloAnimation.module.css';

export default function HelloAnimation({ onComplete }) {
  const [fadeOut, setFadeOut] = useState(false);

  useEffect(() => {
    // Wait for animation to complete (5s) + small delay, then start fade out
  ...
  return (
    ...
    >
      <div className={styles.helloDiv}>
        <svg className={styles.helloSvg} viewBox="0 0 1230.94 414.57">
          <path 
